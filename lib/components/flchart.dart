...
class StatisticBarChart extends StatefulWidget {
  StatisticBarChart({this.providerData, this.dateRange});
...
class StatisticBarChartState extends State<StatisticBarChart> {
  ...
  Widget build(BuildContext context) {
    _getMaxAmount(widget.providerData);
    return BarChart(
      BarChartData(
          ...
          titlesData: FlTitlesData(
            ...
            leftTitles: SideTitles(
              ...
              textStyle: TextStyle(
                  color: const Color(0xff7589a2), fontWeight: FontWeight.bold, fontSize: 14),
              interval: _getInterval(widget.providerData),
            ),
          ),
...
//        _createBarGroupData(widget.providerData),
  ...
  void _getMaxAmount(List<Transfer> dataSet) {
    _maxAmount = 0;
    if (widget.providerData.isEmpty) {
      return;
    }
  ...
  List<BarChartGroupData> _createBarGroupData(List<Transfer> dataSet) {
    List<BarChartGroupData> chartGroupData = [];

    dataSet.forEach((element) {
      return BarChartGroupData(x: 0, barRods: [
        BarChartRodData(
            y: widget.providerData.isEmpty ? 0 : widget.providerData[0].amount,
            width: _barWidth,
